...
def create_expenditure_table(ind):
    rows = {
...
def create_tax_heads_table(ind):
    rows = {
        "Tax revenue": ind["tax_revenue"],
...
def plot_fiscal_dashboard(ind, years, save_path=None):
    ...
    # 7. Revenue table
    add_table(axes[2, 0], create_revenue_table(ind), "Revenue, US$ millions")

    # 8. Expenditure table
    add_table(axes[2, 1], create_expenditure_table(ind), "Expenditure, US$ millions")

    # 9. Tax heads table
    add_table(axes[2, 2], create_tax_heads_table(ind), "Tax revenue heads, US$ millions")
